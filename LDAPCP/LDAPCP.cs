...
namespace ldapcp
{
    ...
    public class LDAPCP : SPClaimProvider
    {
        ...
        protected bool Initialize(Uri context, string[] entityTypes)
        {
            ...
            lock (Sync_Init)
            {
                ...
                try
                {
                    LdapcpLogging.Log($"[{ProviderInternalName}] Refreshing configuration from PersistedObject '{PersistedObjectName}' version {((SPPersistedObject)globalConfiguration).Version.ToString()}...",
                        TraceSeverity.Verbose, EventSeverity.Information, LdapcpLogging.Categories.Core);
                    ...
                    this.CurrentConfiguration = ((LDAPCPConfig)globalConfiguration).CloneInReadOnlyObject();
                    ...
                    //this.CurrentConfiguration.ClaimTypes = new ClaimTypeConfigCollection();
                    ...
                    if (this.CurrentConfiguration.ClaimTypes == null)
                    {
                        ...
                        LdapcpLogging.Log(String.Format("[{0}] ClaimTypes was not set to null in SetCustomConfiguration, don't set it or set it with actual entries.", ProviderInternalName), TraceSeverity.Unexpected, EventSeverity.Error, LdapcpLogging.Categories.Core);
                        return false;
                    }
        ...
        private bool InitializeClaimTypeConfigList(ClaimTypeConfigCollection nonProcessedClaimTypes)
        {
            ...
            try
            {
                ...
                List<ClaimTypeConfig> claimTypesDefinedInTrust = new List<ClaimTypeConfig>();
                ...
                foreach (SPTrustedClaimTypeInformation claimTypeInformation in SPTrust.ClaimTypeInformation)
                {
                    ...
                    //FINDTOWHERE
                    IEnumerable<ClaimTypeConfig> attObjectColl = nonProcessedClaimTypes.Where(x =>
                ...
                List<ClaimTypeConfig> additionalClaimTypes = new List<ClaimTypeConfig>();
                foreach (var attr in nonProcessedClaimTypes.Where(x => x.CreateAsIdentityClaim && !claimTypesDefinedInTrust.Contains(x, new LDAPPropertiesComparer())))
                {
                    if (String.Equals(SPTrust.IdentityClaimTypeInformation.MappedClaimType, attr.ClaimType))
                    {
                        ...
                        LdapcpLogging.Log(String.Format("[{0}] Object with LDAP attribute/class {1}/{2} is set with CreateAsIdentityClaim to true and ClaimTypeProp {3}. Remove ClaimTypeProp property as it is useless.", ProviderInternalName, attr.LDAPAttribute, attr.LDAPClass, attr.ClaimType), TraceSeverity.Monitorable, EventSeverity.Information, LdapcpLogging.Categories.Core);
                    }
                    ...
                    if (attr.LDAPClass == IdentityClaimTypeConfig.LDAPClass)
                    {
                        ...
                        attr.LDAPAttributeToShowAsDisplayText = IdentityClaimTypeConfig.LDAPAttributeToShowAsDisplayText; // Must be set otherwise display text of permissions will be inconsistent
                    }
                    else
                    {
                        // Attribute will be populated with first attribute that matches the LDAP class (and !CreateAsIdentityClaim)
                        var attrReference = claimTypesDefinedInTrust.FirstOrDefault(x => x.LDAPClass == attr.LDAPClass);
                ...
                this.ProcessedClaimTypesList = new List<ClaimTypeConfig>(claimTypesDefinedInTrust.Count + additionalClaimTypes.Count);
                this.ProcessedClaimTypesList.AddRange(claimTypesDefinedInTrust);
                this.ProcessedClaimTypesList.AddRange(additionalClaimTypes);

                // Parse each attribute to configure its settings from the corresponding claim types defined in the SPTrustedLoginProvider
                foreach (var attr in this.ProcessedClaimTypesList)
                {
                ...
                this.ClaimTypesWithUserMetadata = nonProcessedClaimTypes.Where(x =>
                    ...
                    !String.IsNullOrEmpty(x.LDAPAttribute) &&
                    !String.IsNullOrEmpty(x.LDAPClass));// &&                    x.ClaimEntityType == SPClaimEntityTypes.User);
            }
            catch (Exception ex)
            {
        ...
        public static SPTrustedLoginProvider GetSPTrustAssociatedWithCP(string ProviderInternalName)
        {
            ...
            LdapcpLogging.Log(String.Format("[{0}] Claims provider {0} is not associated with any SPTrustedIdentityTokenIssuer so it cannot create permissions.\r\nVisit http://www.ldapcp.com for installation procedure or set property ClaimProviderName with PowerShell cmdlet Get-SPTrustedIdentityTokenIssuer to create association.", ProviderInternalName), TraceSeverity.High, EventSeverity.Warning, LdapcpLogging.Categories.Core);
            return null;
        }
        ...
        protected override void FillResolve(Uri context, string[] entityTypes, SPClaim resolveInput, List<Microsoft.SharePoint.WebControls.PickerEntity> resolved)
        {
            ...
            SPSecurity.RunWithElevatedPrivileges(delegate ()
            {
                ...
                try
                {
                    RequestInformation infos = new RequestInformation(CurrentConfiguration, RequestType.Validation, ProcessedClaimTypesList, resolveInput.Value, resolveInput, context, entityTypes, null, Int32.MaxValue);
        ...
        protected override void FillResolve(Uri context, string[] entityTypes, string resolveInput, List<Microsoft.SharePoint.WebControls.PickerEntity> resolved)
        {
            ...
            SPSecurity.RunWithElevatedPrivileges(delegate ()
            {
                ...
                try
                {
                    RequestInformation settings = new RequestInformation(CurrentConfiguration, RequestType.Search, ProcessedClaimTypesList, resolveInput, null, context, entityTypes, null, Int32.MaxValue);
        ...
        protected override void FillSearch(Uri context, string[] entityTypes, string searchPattern, string hierarchyNodeID, int maxCount, Microsoft.SharePoint.WebControls.SPProviderHierarchyTree searchTree)
        {
            ...
            SPSecurity.RunWithElevatedPrivileges(delegate ()
            {
                ...
                try
                {
                    RequestInformation settings = new RequestInformation(CurrentConfiguration, RequestType.Search, ProcessedClaimTypesList, searchPattern, null, context, entityTypes, hierarchyNodeID, maxCount);
                    List<PickerEntity> permissions = SearchOrValidate(settings);
            ...
            try
            {
                if (this.CurrentConfiguration.BypassLDAPLookup)
                {
                    ...
                    List<PickerEntity> entities = CreatePickerEntityForSpecificClaimTypes(
                        requestInfo.Input,
                        requestInfo.ClaimTypesConfigList.Where(x => !x.CreateAsIdentityClaim),
                        false);
                    if (entities != null)
                    {
                ...
                {
                    //FINDTOWHERE
                    IEnumerable<ClaimTypeConfig> attribsMatchInputPrefix = requestInfo.ClaimTypesConfigList.Where(x =>
        ...
        protected virtual void SearchOrValidateWithLDAP(RequestInformation requestInfo, ref List<PickerEntity> permissions)
        {
            ...
            if (results.Count > 0)
            {
                ...
                if (requestInfo.RequestType == RequestType.Validation
                    && requestInfo.IdentityClaimTypeConfig.ClaimValuePrefix != null)
                {
                    // Extract just the domain from the input
                    bool tokenFound = false;
                    string domainOnly = String.Empty;
                    if (requestInfo.IdentityClaimTypeConfig.ClaimValuePrefix.Contains(Constants.LDAPCPCONFIG_TOKENDOMAINNAME))
                    {
                        tokenFound = true;
                        domainOnly = RequestInformation.GetDomainFromFullAccountName(requestInfo.IncomingEntity.Value);
                    }
                    else if (requestInfo.IdentityClaimTypeConfig.ClaimValuePrefix.Contains(Constants.LDAPCPCONFIG_TOKENDOMAINFQDN))
                    {
                        tokenFound = true;
        ...
        protected virtual ConsolidatedResultCollection ProcessLdapResults(RequestInformation requestInfo, ref List<LDAPSearchResultWrapper> LDAPSearchResultWrappers)
        {
            ...
            //FINDTOWHERE
            if (requestInfo.ExactSearch) attributes = requestInfo.ClaimTypesConfigList.Where(x => !x.CreateAsIdentityClaim);
            ...
            foreach (LDAPSearchResultWrapper LDAPresult in LDAPSearchResultWrappers)
            {
                ...
                foreach (var attr in attributes)
                {
                    ...
                    if (attr.CreateAsIdentityClaim && (String.Equals(attr.LDAPClass, IdentityClaimTypeConfig.LDAPClass, StringComparison.InvariantCultureIgnoreCase)))
                    {
                        if (!resultPropertyCollectionPropertyNames.Contains(attr.LDAPAttribute, StringComparer.InvariantCultureIgnoreCase)) continue;
                    ...
                    bool compareWithDomain = HasPrefixToken(attr.ClaimValuePrefix, Constants.LDAPCPCONFIG_TOKENDOMAINNAME) ? true : this.CurrentConfiguration.CompareResultsWithDomainNameProp;
                    if (!compareWithDomain) compareWithDomain = HasPrefixToken(attr.ClaimValuePrefix, Constants.LDAPCPCONFIG_TOKENDOMAINFQDN) ? true : this.CurrentConfiguration.CompareResultsWithDomainNameProp;
        ...
        protected bool QueryLDAPServers(List<LDAPConnectionSettings> LDAPServers, RequestInformation requestInfo, ref List<LDAPSearchResultWrapper> LDAPSearchResults)
        {
            ...
            Parallel.ForEach(LDAPServers, LDAPServer =>
            {
                ...
                using (DirectorySearcher ds = new DirectorySearcher(LDAPServer.Filter))
                {
                    ...
                    foreach (var metadataAttribute in ClaimTypesWithUserMetadata)
                    {
        ...
        protected virtual LDAPConnection[] GetLDAPServers(RequestInformation requestInfo)
        {
            ...
            foreach (var ldapConnection in ldapConnections)
            {
                ...
                else
                {
                    ...
                    LdapcpLogging.Log(String.Format("[{0}] Add AD server \"{1}\" with AuthenticationType \"{2}\" and credentials of application pool account.", ProviderInternalName, coco.LDAPServer.Path, coco.LDAPServer.AuthenticationType.ToString()), TraceSeverity.Verbose, EventSeverity.Information, LdapcpLogging.Categories.LDAP_Lookup);
                }
                connections[i++] = coco;
            }
            return connections;
        ...
        protected override void FillClaimTypes(List<string> claimTypes)
        {
            ...
            LdapcpLogging.LogDebug(String.Format("[{0}] FillClaimValueTypes called, ProcessedAttributes null: {1}", ProviderInternalName, ProcessedClaimTypesList == null ? true : false));
        ...
        protected virtual void AugmentEntity(Uri context, SPClaim entity, SPClaimProviderContext claimProviderContext, List<SPClaim> claims)
        {
            ...
            SPSecurity.RunWithElevatedPrivileges(delegate ()
            {
                ...
                try
                {
                    ...
                    var groupAttribute = this.ProcessedClaimTypesList.FirstOrDefault(x => String.Equals(x.ClaimType, this.CurrentConfiguration.ClaimTypeUsedForAugmentation, StringComparison.InvariantCultureIgnoreCase) && !x.CreateAsIdentityClaim);
        ...
        /// <returns></returns>
        protected virtual List<SPClaim> GetGroupsFromADDirectory(DirectoryEntry directory, RequestInformation requestInfo, ClaimTypeConfig groupAttribute)
        {
            List<SPClaim> groups = new List<SPClaim>();
                ...
                try
                {
                    ...
                    using (PrincipalContext principalContext = new PrincipalContext(ContextType.Domain, directoryDomainFqdn))
                    {
                        ...
                        foreach (Principal group in ADGroups)
                        {
                            ...
                            if (!String.IsNullOrEmpty(groupAttribute.ClaimValuePrefix) && groupAttribute.ClaimValuePrefix.Contains(Constants.LDAPCPCONFIG_TOKENDOMAINNAME))
                                claimValue = groupAttribute.ClaimValuePrefix.Replace(Constants.LDAPCPCONFIG_TOKENDOMAINNAME, groupDomainName) + group.Name;
                            else if (!String.IsNullOrEmpty(groupAttribute.ClaimValuePrefix) && groupAttribute.ClaimValuePrefix.Contains(Constants.LDAPCPCONFIG_TOKENDOMAINFQDN))
                                claimValue = groupAttribute.ClaimValuePrefix.Replace(Constants.LDAPCPCONFIG_TOKENDOMAINFQDN, groupDomainFqdn) + group.Name;
        ...
        /// <param name="groupAttribute"></param>
        /// <returns></returns>
        protected virtual List<SPClaim> GetGroupsFromLDAPDirectory(DirectoryEntry directory, RequestInformation requestInfo, ClaimTypeConfig groupAttribute)
        {
            List<SPClaim> groups = new List<SPClaim>();
                ...
                try
                {
                    ...
                    using (DirectorySearcher searcher = new DirectorySearcher(directory))
                    {
                        ...
                        for (int propertyCounter = 0; propertyCounter < propertyCount; propertyCounter++)
                        {
                            ...
                            if (!String.IsNullOrEmpty(groupAttribute.ClaimValuePrefix) && groupAttribute.ClaimValuePrefix.Contains(Constants.LDAPCPCONFIG_TOKENDOMAINNAME))
                                claimValue = groupAttribute.ClaimValuePrefix.Replace(Constants.LDAPCPCONFIG_TOKENDOMAINNAME, groupDomainName) + claimValue;
                            else if (!String.IsNullOrEmpty(groupAttribute.ClaimValuePrefix) && groupAttribute.ClaimValuePrefix.Contains(Constants.LDAPCPCONFIG_TOKENDOMAINFQDN))
                                claimValue = groupAttribute.ClaimValuePrefix.Replace(Constants.LDAPCPCONFIG_TOKENDOMAINFQDN, groupDomainFqdn) + claimValue;
                            SPClaim claim = CreateClaim(groupAttribute.ClaimType, claimValue, groupAttribute.ClaimValueType, false);
                            groups.Add(claim);
                        }
                    }
        ...
        {
            LdapcpLogging.LogDebug(String.Format("[{0}] FillEntityTypes called, ProcessedAttributes null: {1}", ProviderInternalName, ProcessedClaimTypesList == null ? true : false));
            ...
            try
            {
                var spUniqueEntitytypes = from attributes in ProcessedClaimTypesList
        ...
        protected override void FillHierarchy(Uri context, string[] entityTypes, string hierarchyNodeID, int numberOfLevels, Microsoft.SharePoint.WebControls.SPProviderHierarchyTree hierarchy)
        {
            ...
            SPSecurity.RunWithElevatedPrivileges(delegate ()
            {
                ...
                try
                {
                    ...
                    {
                        // Root level
                        foreach (var attribute in ProcessedClaimTypesList.Where(x => !x.CreateAsIdentityClaim && entityTypes.Contains(x.ClaimEntityType)))
                        {
                            hierarchy.AddChild(
        ...
        protected virtual SPClaim CreateClaim(string type, string value, string valueType, bool inputHasKeyword)
        {
            ...
            if ((!inputHasKeyword || !attr.DoNotAddClaimValuePrefixIfBypassLookup) &&
                !HasPrefixToken(attr.ClaimValuePrefix, Constants.LDAPCPCONFIG_TOKENDOMAINNAME) &&
                !HasPrefixToken(attr.ClaimValuePrefix, Constants.LDAPCPCONFIG_TOKENDOMAINFQDN)
            )
        ...
        protected virtual PickerEntity CreatePickerEntityHelper(ConsolidatedResult result)
        {
            ...
            if ((String.Equals(permissionClaimType, SPTrust.IdentityClaimTypeInformation.MappedClaimType, StringComparison.InvariantCultureIgnoreCase)
                || result.Attribute.CreateAsIdentityClaim) && result.Attribute.LDAPClass == IdentityClaimTypeConfig.LDAPClass)
            {
                isIdentityClaimType = true;
            }

            if (result.Attribute.CreateAsIdentityClaim && result.Attribute.LDAPClass != IdentityClaimTypeConfig.LDAPClass)
            {
                // Get reference attribute to use to create actual permission (claim type and its LDAPAttribute) from current result
                ClaimTypeConfig attribute = ProcessedClaimTypesList.FirstOrDefault(x => !x.CreateAsIdentityClaim && x.LDAPClass == result.Attribute.LDAPClass);
                if (attribute != null)
                {
            ...
            foreach (var entityAttrib in ClaimTypesWithUserMetadata.Where(x => String.Equals(x.LDAPClass, result.Attribute.LDAPClass, StringComparison.InvariantCultureIgnoreCase)))
            {
        ...
        protected virtual string FormatPermissionValue(string claimType, string claimValue, string domainName, string domainFQDN, bool isIdentityClaimType, ConsolidatedResult result)
        {
            ...
            if (HasPrefixToken(attr.ClaimValuePrefix, Constants.LDAPCPCONFIG_TOKENDOMAINNAME))
                value = string.Format("{0}{1}", attr.ClaimValuePrefix.Replace(Constants.LDAPCPCONFIG_TOKENDOMAINNAME, domainName), value);

            if (HasPrefixToken(attr.ClaimValuePrefix, Constants.LDAPCPCONFIG_TOKENDOMAINFQDN))
                value = string.Format("{0}{1}", attr.ClaimValuePrefix.Replace(Constants.LDAPCPCONFIG_TOKENDOMAINFQDN, domainFQDN), value);
        ...
        protected virtual string FormatPermissionDisplayText(string claimType, string claimValue, bool isIdentityClaimType, ConsolidatedResult result, PickerEntity entityInfo)
        {
            ...
            if (HasPrefixToken(attr.ClaimValuePrefix, Constants.LDAPCPCONFIG_TOKENDOMAINNAME))
                prefixToAdd = string.Format("{0}", attr.ClaimValuePrefix.Replace(Constants.LDAPCPCONFIG_TOKENDOMAINNAME, result.DomainName));

            if (HasPrefixToken(attr.ClaimValuePrefix, Constants.LDAPCPCONFIG_TOKENDOMAINFQDN))
                prefixToAdd = string.Format("{0}", attr.ClaimValuePrefix.Replace(Constants.LDAPCPCONFIG_TOKENDOMAINFQDN, result.DomainFQDN));
    ...
    /// </summary>
    class LDAPPropertiesComparer : IEqualityComparer<ClaimTypeConfig>
    {
        // LDAP Attributes are equal if they have same LDAPAttribute and same LDAPObjectClass
        public bool Equals(ClaimTypeConfig x, ClaimTypeConfig y)
        {
            ...
            // Check if they have same LDAPAttribute and same LDAPObjectClass
            //return ((String.Compare(x.LDAPAttribute, y.LDAPAttribute, true) == 0) && (String.Compare(x.LDAPObjectClass, y.LDAPObjectClass, true) == 0));
            return ((String.Equals(x.LDAPAttribute, y.LDAPAttribute, StringComparison.OrdinalIgnoreCase)) && (String.Equals(x.LDAPClass, y.LDAPClass, StringComparison.OrdinalIgnoreCase)));
        }

        // If Equals() returns true for a pair of objects 
        // then GetHashCode() must return the same value for these objects.
        public int GetHashCode(ClaimTypeConfig attribute)
        {
            ...
            return (attribute.LDAPAttribute + " " + attribute.LDAPClass).GetHashCode();
        }
    }
