...
namespace Yvand.Config
{
    ...
    public class OperationContext
    {
        ...
        public List<LDAPConnection> AzureTenants { get; private set; }
        ...
        public OperationContext(IAzureCPSettings settings, OperationType currentRequestType, string input, SPClaim incomingEntity, Uri context, string[] entityTypes, string hierarchyNodeID, int maxCount)
        {
            ...
            this.AzureTenants = new List<LDAPConnection>(settings.AzureTenants.Count);
            foreach (LDAPConnection tenant in settings.AzureTenants)
            {
        ...
        protected void InitializeValidation(List<ClaimTypeConfig> runtimeClaimTypesList)
        {
            ...
            {
                Logger.Log($"[{LDAPCP.ClaimsProviderName}] Unable to validate entity \"{this.IncomingEntity.Value}\" because its claim type \"{this.IncomingEntity.ClaimType}\" was not found in the ClaimTypes list of current configuration.", TraceSeverity.Unexpected, EventSeverity.Error, TraceCategory.Configuration);
                throw new InvalidOperationException($"[{LDAPCP.ClaimsProviderName}] Unable validate entity \"{this.IncomingEntity.Value}\" because its claim type \"{this.IncomingEntity.ClaimType}\" was not found in the ClaimTypes list of current configuration.");
            }
        ...
        protected void InitializeAugmentation(List<ClaimTypeConfig> runtimeClaimTypesList)
        {
            ...
            if (this.IncomingEntityClaimTypeConfig == null)
            {
                Logger.Log($"[{LDAPCP.ClaimsProviderName}] Unable to augment entity \"{this.IncomingEntity.Value}\" because its claim type \"{this.IncomingEntity.ClaimType}\" was not found in the ClaimTypes list of current configuration.", TraceSeverity.Unexpected, EventSeverity.Error, TraceCategory.Configuration);
                throw new InvalidOperationException($"[{LDAPCP.ClaimsProviderName}] Unable to augment entity \"{this.IncomingEntity.Value}\" because its claim type \"{this.IncomingEntity.ClaimType}\" was not found in the ClaimTypes list of current configuration.");
            }
        }
    }
}
